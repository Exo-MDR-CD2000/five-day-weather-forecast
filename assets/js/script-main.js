...
function parseForecastData(currentForecast) {
  ...
  currentForecast.list.forEach((weatherData) => {
    ...
    // Add the temperature, wind speed, humidity, and icon id to the dailyWeather object
    ...
    dailyWeather[date].iconId = weatherData.weather[0].icon;
  });
  ...
  // var iconUrlForecast = `https://openweathermap.org/img/wn/${iconId}@4x.png`;
  // console.log('iconUrlForecast', iconUrlForecast);
  ...
  //var cli = document.querySelector('#icon-day-1').style.display = 'block'; // ci1 = current icon 1
